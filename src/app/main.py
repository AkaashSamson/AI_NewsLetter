...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Run Polling Cycle"):
            try:
                with st.spinner("Polling feeds & Generating summaries... (Max 5 items)"):
    ...
    with col2:
        st.metric("Total AI Summaries", len(summaries)) # This logic is technically "Latest" length, but indicative for now. 
    ...
    with tab1:
        ...
        for item in summaries:
            with st.expander(f"{item.title}  |  {item.channel_id}", expanded=True):
                ...
                st.caption(f"Published: {item.published_at}  •  [Watch Video]({item.link})")
