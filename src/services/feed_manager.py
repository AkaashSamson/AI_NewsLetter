...
class FeedManager:
    ...
    def __init__(self, db: Session):
        ...
        self.writer = LLMWriter(config_path="src/config/config.yaml") # TODO: Refactor config path
    ...
    def run_polling_cycle(self):
        ...
        logger.info(f"Found {len(candidates)} new candidates across {len(channels)} channels.")

        # 3. Apply Global Rate Limit
        limit = settings.MAX_VIDEOS_PER_RUN
        if len(candidates) > limit:
            logger.warning(f"Rate Limiting: Processing only {limit} of {len(candidates)} videos.")
    ...
    def _process_single_video(self, channel: models.Channel, video: VideoMetadata):
        ...
        summary_result = self.writer.process_content(video.title, transcript_data.clean_text)
        ...
        crud.create_summary(
            ...
            link=video.link,
            published_at=datetime.fromisoformat(video.published_at.replace("Z", "+00:00")) # Basic parsing fallback
        )
        
        # Update Channel Timestamp
        # We only update if this video is newer than current last_checked
        # Robust parsing needed here, but for now simple check:
        # crud.update_channel_last_checked(self.db, channel.channel_id, ...)
