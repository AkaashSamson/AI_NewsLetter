...
class ChannelManager:
    ...
    def add_new_channel(self, url: str) -> str:
        ...
        # 2. Resolve ID
        try:
            channel_id = self.resolver.get_channel_id(url)
            logger.info(f"Resolved {url} -> {channel_id}")
        except Exception as e:
            logger.error(f"Failed to resolve channel ID for {url}: {e}")
            raise ValueError(f"Could not resolve channel ID: {e}")
        ...
        # 4. Save
        # Note: We don't fetch the name yet, we can do that lazily or add a fetcher later.
        # For now, name is None.
        crud.create_channel(self.db, channel_id=channel_id, url=url)
        return f"Successfully added channel: {channel_id}"
